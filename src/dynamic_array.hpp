		...
		public:
			...
			void* push() {
				if (m_count == m_capacity) {
					m_data = (u8*)r2realloc(m_data, m_capacity * m_elementSize, m_capacity * 2 * m_elementSize);
					m_capacity *= 2;
				}
			...
			void remove(size_t index) {
				...
				if (index == m_count) {
					if (m_count < m_capacity / 2 && m_capacity > 16) {
						m_capacity /= 2;
						m_data = (u8*)r2realloc(m_data, m_count * m_elementSize, m_capacity * m_elementSize);
					}
					return;
				}
			...
			void clear() {
				...
				if (m_capacity != 16) {
					m_capacity = 16;
					r2realloc(m_data, 0, m_elementSize * 16);
				}
			}
	...
	class dynamic_pod_array {
		public:
			...
			void push(const T& value) {
				if (m_count == m_capacity) {
					m_data = (T*)r2realloc(m_data, m_capacity * sizeof(T), m_capacity * 2 * sizeof(T));
					m_capacity *= 2;
				}
			...
			void remove(size_t index) {
				...
				if (index == m_count) {
					if (m_count < m_capacity / 2 && m_capacity > 16) {
						m_capacity /= 2;
						m_data = (T*)r2realloc(m_data, m_count * sizeof(T), m_capacity * sizeof(T));
					}
					return;
				}
			...
			void clear() {
				...
				if (m_capacity != 16) {
					m_capacity = 16;
					r2realloc(m_data, 0, sizeof(T) * 16);
				}
			}
