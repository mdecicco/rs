...
namespace rs {
	...
	execution_state::execution_state(const context_parameters& params, context* ctx) {
		...
		m_stack = (variable(*)[rc])new variable[rc * m_stack_depth];
		...
		while (iaddr < exit_point) {
			...
			if (instruction.arg_count > 0) {
				if (instruction.arg_is_register[0]) {
					iset = m_ctx->get_instruction_set(registers[instruction.args[0].reg].type());
				} else {
					iset = m_ctx->get_instruction_set(m_ctx->memory->get(instruction.args[0].var).type());
				}
			}
	...
	void execution_state::push_state() {
		...
		variable* prev = m_stack[m_stack_idx++];
		variable* now = m_stack[m_stack_idx];
		now[rs_register::lvalue].set(prev[rs_register::lvalue]);
		now[rs_register::rvalue].set(prev[rs_register::rvalue]);
		now[rs_register::this_obj].set(prev[rs_register::this_obj]);
		for (u8 x = 0;x < 8;x++) now[rs_register::parameter0 + x].set(prev[rs_register::parameter0 + x]);
		now[rs_register::instruction_address].set(prev[rs_register::instruction_address]);
		now[rs_register::return_address].set((integer_type)rs_integer_max);
	}
	...
	void execution_state::pop_state(rs_register persist) {
		...
		variable* prev = m_stack[m_stack_idx--];
		variable* now = m_stack[m_stack_idx];
		now[rs_register::return_value].set(prev[rs_register::return_value]);
		now[rs_register::instruction_address].set(prev[rs_register::instruction_address]);
		now[persist].set(prev[persist]);
		memset(prev, 0, rs_register::register_count * sizeof(variable));
	}
	...
	void execution_state::print_instruction(integer_type idx, const instruction_array::instruction& i) {
		...
		for (int a = 0;a < i.arg_count;a++) {
			if (i.arg_is_register[a]) {
				variable& reg = registers()[i.args[a].reg];
				printf(" $%s (%s)", rstr[i.args[a].reg], reg.to_string().c_str());
			}
			else {
				auto& v = m_ctx->memory->get(i.args[a].var);
				printf(" #%llu (%s)", i.args[a].var, v.to_string().c_str());
			}
		}
		printf(" [stack: %d]\n", m_stack_idx);
	}
